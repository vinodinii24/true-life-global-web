...
export default function Testimonials() {
  return (
    <section 
      id="testimonials" 
      className="relative bg-transparent text-[#0F172A] py-24 sm:py-32 overflow-hidden border-t border-slate-200/60"
        ...
        >
          {testimonialsData.map((item, index) => (
            <motion.article
              ...
              className="group relative bg-white border border-[#E2E8F0] p-8 rounded-3xl shadow-[0_10px_30px_rgba(15,23,42,0.015)] flex flex-col justify-between transition-all duration-300 hover:border-slate-300 hover:shadow-[0_20px_50px_rgba(15,23,42,0.04)]"
            >
              {/* Card Ribbon Accent */}
              <div className="absolute top-0 left-0 right-0 h-[3px] bg-slate-100 group-hover:bg-[#1D4ED8] rounded-t-3xl transition-colors duration-300" />

              <div>
                {/* Header Node: Quote & Rating Matrix */}
                <div className="flex justify-between items-center mb-6">
                  <div className="w-10 h-10 rounded-xl bg-slate-50 border border-slate-100 flex items-center justify-center text-[#1D4ED8] group-hover:bg-[#1D3557] group-hover:text-white transition-colors duration-300">
                ...
                <p className="font-sans text-base text-slate-600 leading-relaxed mb-8 font-normal italic">
                  "{item.review}"
                </p>
              </div>
              ...
              <div className="pt-6 border-t border-slate-100 flex items-center justify-between">
                <div>
                  <h3 className="font-sans text-base font-bold text-[#1D3557] tracking-tight">
                    {item.name}
                  </h3>
                ...
                <div className="w-1.5 h-1.5 rounded-full bg-slate-100 group-hover:bg-[#D4AF37] transition-colors duration-300" />
              </div>
